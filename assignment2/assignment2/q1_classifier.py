...
from tensorflow.python.training.gradient_descent import \
    GradientDescentOptimizer

from model import Model
from q1_softmax import cross_entropy_loss, softmax
...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        """Adds the core transformation for this model which transforms a
        batch of input data into a batch of predictions. In this case, the
    ...
    def fit(self, sess, inputs, labels):
        ...
        for epoch in range(self.config.n_epochs):
            ...
            print('Epoch {:}: loss = {:.2f} ({:.3f} sec)'
                  ''.format(epoch, average_loss, duration))
...
def test_softmax_model():
    ...
    # If Ops are implemented correctly, the average loss should fall close
    # to zero rapidly.
    assert losses[-1] < .5
